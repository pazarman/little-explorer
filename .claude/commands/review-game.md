---
description: Critically score a mini-game against the toddler quality bar and propose upgrades to reach 24/24.
...
1. Read `docs/skills/01-product-quality-bar.md` (the v2 12-axis rubric), plus `02-learning-design`, `03-content-quality`, `04-accessibility-safety`, and `06-stem-scope-and-sequence`.
...
3. Score each of the 12 axes 0/1/2 using the anchors, each with a one-line justification and exact `file:line` evidence.
4. Give the total and a ship / no-ship call against the 18/24 threshold and the auto-fail rules.
5. Tag the game's STEM objective (Domain · Concept · Age band · Success) per skill 06, and flag if it's weak or missing.
6. List the 2–4 highest-ROI upgrades to reach 24/24, each tied to a specific axis or STEM gap.
