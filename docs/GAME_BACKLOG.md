...
### [PROPOSED] Hot or Cold?
- **STEM:** Measurement / comparison · temperature as a measurable property (hot / cold / warm / cool) · age 2–4
- **Success:** Child can drag everyday objects (steaming soup, ice cube, warm blanket, frozen treat, sunny rock, snow) onto a hot side (sun icon) or a cold side (snowflake icon), watch an animated thermometer rise or drop, and hear the temperature word — demonstrating that temperature is a measurable, sortable property distinct from size or weight
- **Fills gap:** Measurement/comparison — the STEM doc marks Measurement as a Gap. The backlog already addresses height (measure.js, DONE), capacity (pour.js, READY), weight (balance.js, PROPOSED), and seriation (lineup.js, PROPOSED). **Temperature (hot/cold)** is absent from every existing game file and from all backlog entries. Temperature is the most sensory-accessible measurement concept for ages 2–4 — toddlers encounter hot/cold constantly (bath water, food, weather) and already use the words. It is a canonical early-STEM sorting task in every preschool curriculum.
- **Rubric focus:** Learning efficacy (thermometer animation makes temperature concrete and continuous — bar rises on hot objects, drops on cold; voice names the property on each placement: "The soup is HOT — it makes the thermometer go way up!"; the causal link between object property and thermometer movement is shown, not stated), Interaction (real-time drag: each object emits a visual cue — steam wisps on hot items, ice crystals on cold — before child commits to a side), Pacing (tier 0 = 2 maximally obvious objects, ice cube vs. steaming soup, two large drop zones; tier 1 = 4 objects including "warm" items like a cozy blanket with a mid-range thermometer result; tier 2 = 5 objects where child sorts into 3 zones: HOT / WARM / COLD — introducing the idea that temperature is a spectrum, not binary)
- **Estimated complexity:** Low-Medium — SVG thermometer with animated fill level; 2 large drop zones (+ a third at tier 2); 6–8 SVG object cards each tagged with a temperature value; per-zone voice line + thermometer animation; design challenge is making warm vs. cold vs. hot visually distinct without being a test
- **File:** `js/games/hotcold.js`

### [PROPOSED] What Am I? (Clue Detective)
- **STEM:** Logic / cause–effect · deductive exclusion — narrow down to one answer using sequential clues · age 4–5
- **Success:** Child can listen to 2–3 voice clues delivered one at a time ("I have four legs… I live on a farm… I say MOO") and tap the correct answer from three picture choices — demonstrating they can hold multiple properties in mind, apply each as a filter, and converge on a conclusion
- **Fills gap:** Logic/cause-effect, 4–5 age band. Chain Reaction covers the 2–3 tier (tap→reaction); What Comes Next? covers the 3–4 tier (sequence/order); What Happens If? covers 4–5 if-then prediction. **Deductive reasoning** — accumulating clues and narrowing possibilities — is a distinct 4–5 logical operation: child must reason "it could be a dog OR a cow after clue 1, but only the cow lives on a farm AND says moo." This convergent multi-clue reasoning is absent from every backlog entry and every existing game file. It is the canonical "riddle" genre that pre-K curricula use to build inferential thinking.
- **Rubric focus:** Learning efficacy (clues are voiced one at a time with a pause — child is free to tap as soon as they're confident; wrong early guesses reset to "let's hear more clues!" with no penalty, teaching that more information leads to better answers), Clarity (three large picture choices stay visible throughout; the silhouette of the answer slowly reveals as clues mount — giving a visual build-up that mirrors the deductive narrowing), Pacing (tier 0 = 2 clues, maximally contrasting answer choices e.g. fish vs. cow vs. airplane; tier 1 = 3 clues with two plausible candidates after clue 1; tier 2 = 3 clues where child must hold a subtle distinguishing property — "I have wings but I'm NOT a bird")
- **Estimated complexity:** Low-Medium — 3 large picture choice cards; progressive-reveal silhouette animation (CSS clip-path expanding per clue); 10–12 curated riddle sets (each a list of 2–3 voice clues + answer); voice pacing is the core design challenge (pause long enough that the child can reason, but short enough to maintain curiosity); no drag, no reading required
- **File:** `js/games/clues.js`

### [PROPOSED] All Together Now!
- **STEM:** Social-emotional learning (SEL) · sharing + cooperative achievement — contributing to a shared goal · age 3–5
- **Success:** Child can take turns with a buddy character — buddy adds a piece, then child taps to add their piece — to build a shared structure (snowman, sandcastle, tower of blocks), and celebrate the finished object together — demonstrating that giving up something (a piece) to a shared goal produces a better outcome than acting alone
- **Fills gap:** SEL, 3–4 and 4–5 age bands. Feelings Friends covers emotion naming and cause-of-feelings (2–4); Pass the Smile covers turn-taking at the 2–3 level (attending and waiting); How Does Bunny Feel? covers perspective-taking (4–5); Wind-Down Corner covers self-regulation. **Sharing toward a cooperative goal** — the child literally contributes a resource (gives a piece to the shared structure) and both the buddy and child benefit — is the distinct 3–4 SEL milestone, treated as first-class by Daniel Tiger ("sharing is caring"), Sago Mini (co-op activities), and PBS Kids. It is absent from every backlog entry and every existing game file. Different from turn-taking (Pass the Smile): here both parties contribute to the SAME object and the child must willingly give something away.
- **Rubric focus:** Emotional safety (no fail state — buddy always says "thank you!" warmly on the child's contribution; partial structures are celebrated as "we're building it together!"; voice never pressures), Learning efficacy (voice makes the causal link explicit: "You shared a piece — look, it's getting taller! When we share, we build MORE together!"; the finished structure is visibly bigger than either character could have built alone — cooperation produces a better outcome), Pacing (tier 0 = 2-piece structure, one each, with strong visual cue highlighting child's piece before their turn; tier 1 = 4-piece alternating build; tier 2 = child can choose WHICH piece to give from two options, introducing intentional generosity over compliance)
- **Estimated complexity:** Low — 3 SVG build themes (snowman: head/body, sandcastle: tower/flag, block tower: base/top); animated piece-drop with a sparkle + warm buddy reaction; simple alternating-turn engine (buddy auto-places after child's turn); voice script per piece contribution; design challenge is making the SHARED celebration (both child and buddy cheer) feel genuinely reciprocal rather than performative
- **File:** `js/games/together.js`

### [PROPOSED] Heavy or Light?
- **STEM:** Measurement / comparison · weight comparison (heavy / light) · age 2–4
- **Success:** Child can drag objects onto two pans of a balance scale, observe which side tips down, and tap the heavier or lighter one when asked — demonstrating that weight is a measurable property distinct from size, and that a scale reveals which object has more weight
- **Fills gap:** Measurement/comparison, 2–3 and 3–4 age bands. The STEM doc marks Measurement as a Gap, but every existing proposal addresses a different sub-concept: Tall or Short (DONE) = height; Line Them Up = length seriation; More or Less = quantity; Fill It Up! = volume. **Weight (heavy/light)** is absent from both the backlog and all existing game files. A balance scale is the canonical hands-on weight-comparison tool in every preschool classroom. fuelup.js compares countable quantities, not physical weight; sort.js compares size, not mass.
- **Rubric focus:** Learning efficacy (balance scale makes weight kinetic and visible — pans physically tilt on each object drop; voice labels the outcome: "The rock makes the scale tip — it's HEAVIER!"; the causal link between weight and pan movement is shown, not stated), Interaction (real-time drag: each object produces a landing thud and pan-tilt animation — child-driven, physical metaphor kids already know from playground seesaws), Pacing (tier 0 = 2 maximally different objects, feather vs. rock, dramatic pan dip; tier 1 = 4 objects to rank in two pairs; tier 2 = 3 objects where child sorts all three by weight including one near-miss pair where the smaller object is heavier — e.g., a small rock vs. a large balloon — teaching that size ≠ weight)
- **Estimated complexity:** Low-Medium — SVG balance scale with pivot-animated arm and two pans; 6–8 object SVGs each tagged with weight metadata; pan-tilt via CSS transform on the scale arm; voice delivers weight comparison labels; design challenge is making the weight-vs-size contrast visible and surprising without feeling like a trick
- **File:** `js/games/balance.js`

### [PROPOSED] Color Lab
- **STEM:** Scientific thinking · material properties — color mixing prediction and observation · age 3–5
- **Success:** Child can tap two color droppers into a mixing bowl, predict what color the result will be (by tapping a color tile before the mix), watch the bowl swirl into the new color, and hear WHY — demonstrating that mixing colors follows predictable rules (red + yellow = orange) and that materials can be combined to create new properties
- **Fills gap:** Scientific thinking, 3–4 and 4–5 age bands. The 4–5 tier in the STEM doc is "predict" — Sink or Float? fills physical-property prediction. Color mixing fills **material-change prediction**: child reasons from known inputs to a predicted output, then confirms. This is entirely absent from the backlog and existing games. paint.js is a creative art tool — it does not frame mixing as a science prediction, uses no predict → observe → explain structure, and states no WHY. Color mixing is a canonical preschool science experiment (safe, concrete, sensory, non-reading). No backlog entry covers it.
- **Rubric focus:** Learning efficacy (two-step structure: PREDICT by tapping the expected color tile, then OBSERVE the swirl and hear "Red and yellow make ORANGE — your prediction was right!"; wrong predictions resolve as discovery — "let's see what really happens!"; the mixing rule is named every round so it's learned, not guessed), Interaction (tap-to-drip: each dropper tap adds a visible drop with splat sound and ripple; child taps a swirl button and watches the transformation — satisfying, real-time reveal), Pacing (tier 0 = 3 primary mixes only: R+Y=orange, R+B=purple, B+Y=green, with 2 color tiles to choose from; tier 1 = 6 mixes with 3 choices per round; tier 2 = child chooses WHICH two colors to mix from the dropper tray to match a target color shown on a card — reverses the direction)
- **Estimated complexity:** Medium — SVG mixing bowl with color-change swirl animation (CSS hue-rotate + opacity overlay); 6 dropper SVGs (3 primary colors + 3 secondary); 6-entry mix lookup table; per-mix voice explanation; tier 2 target-match requires backwards reasoning; design challenge is making the swirl animation feel like a genuine reveal rather than an instant switch
- **File:** `js/games/colorlab.js`

### [PROPOSED] Chef Helper
- **STEM:** Logic / cause–effect · multi-step sequencing — execute ordered steps to reach a goal · age 3–5
- **Success:** Child can follow a 2–4 step picture recipe (illustrated steps without words: bowl → add flour → add egg → stir) by performing each step in the correct order via tap or drag — demonstrating that a goal is reached by executing discrete, ordered steps and that reversing or skipping steps produces a wrong outcome
- **Fills gap:** Logic/cause-effect, 3–4 age band. What Comes Next? (PROPOSED, sequence.js) targets **receptive** sequencing — child identifies the next card in a sequence. Chef Helper targets **active/productive** sequencing — child performs each step in order to complete a real goal. These are developmentally distinct: receptive identification ("which card is next?") is earlier-developing than productive execution ("do the next step yourself"). Multi-step goal-directed sequencing is the canonical 3–4 "sequence/order" sub-skill and is absent from all backlog proposals and all existing game files. Animal Hospital covers single-step needs→action→reaction; this game chains 3–4 steps where order is causal.
- **Rubric focus:** Learning efficacy (step progress strip stays visible throughout — child always sees WHERE they are; voice names each step and WHY order matters: "First we add the flour, THEN the egg — mixing them backwards would change the cake!"; completing the recipe reveals a happy animated result), Clarity (only the active step's tap/drag target is enabled — one thing to do at a time; step strip dims completed steps and highlights the current one), Pacing (tier 0 = 2-step recipe, large tap targets, strong visual highlighting; tier 1 = 3-step recipe with lighter cues; tier 2 = 4-step recipe with one wrong-order distractor card that child must drag OUT of the recipe strip before starting)
- **Estimated complexity:** Low-Medium — 4–6 SVG recipe scenes (soup, sandwich, smoothie, cake, salad) each with 2–4 illustrated step cards; step-strip progress UI; active-step gating (disable non-current targets); voice for each step and completion; tier 2 distractor mechanic is the main design challenge; no physics needed
- **File:** `js/games/recipe.js`
